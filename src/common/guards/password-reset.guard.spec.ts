...
describe("PasswordResetGuard", () => {
    ...
    mockConfigService.get.mockImplementation((key: string) => {
      switch (key) {
        case "PASSWORD_RESET_SECRET":
          return "test-password-reset-secret";
        default:
          return undefined;
      }
    });
  ...
  beforeEach(async () => {
    ...
    guard = module.get<PasswordResetGuard>(PasswordResetGuard);
  ...
  it("PasswordResetGuard가 정의되어야 한다.", () => {
    expect(guard).toBeDefined();
  });

  describe("canActivate", () => {
    it("유효한 토큰이 제공되면 true를 반환하고 request 객체에 passwordReset을 추가해야 한다.", async () => {
      const payload = { sub: "password-reset", email: "test@test.com" };
      const mockSecret = configService.get<string>("PASSWORD_RESET_SECRET");
      ...
      expect(mockContext.getRequest().passwordReset).toEqual(payload);
    ...
    it("토큰이 제공되지 않으면 UnauthorizedException을 던져야 한다", async () => {
      ...
      await expect(guard.canActivate(mockContext)).rejects.toThrow(
        new UnauthorizedException("비밀번호 초기화 인증 토큰이 없습니다.")
      );
    });
